<template>
  <article class="container flex flex-col gap-y-8 p-4 border-b-[0.4rem] border-split transition-colors duration-350 ease-out responsive-hover:bg-bg-strong responsive-hover:border-fg-weak">
    <NuxtLink
      :to="item.path"
      class="group/article-list-item grid grid-rows-[3.6rem_2.8rem] gap-y-4"
      data-testid="link">
      <div
        class="text-[1.2rem] leading-18 overflow-clip transition-colors duration-350 ease-out group-responsive-hover/article-list-item:text-fg-strong"
        data-testid="title">
        {{ item.title }}
      </div>
      <div
    ...
    <div class="flex justify-between text-[0.8rem] text-fg-weak">
      <div
        ...
        <NuxtLink
          ...
          class="me-7 transition-colors duration-350 ease-out responsive-hover:underline responsive-hover:text-fg">
          未設定
        </NuxtLink>
        <template v-else>
          <NuxtLink
